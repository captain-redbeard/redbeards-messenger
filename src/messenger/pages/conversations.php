...
if(isset($_POST['m']) && strlen(trim($_POST['m'])) > 0) {
	$result = explode(":", Conversation::sendMessage($guid, $cguid, $_POST['m'], $mysqli));
...
if(isset($menu) && $menu == "new") {
	$newconversation = QB::select("SELECT contact_guid, made_date, (SELECT username FROM users WHERE user_guid = contact_guid) AS username FROM contacts WHERE contact_guid = ? AND user_guid = ?;", 
									array($guid, $_SESSION[USESSION]->user_guid), 
									$mysqli);
}else if(isset($cguid)) {
	$messages = Conversation::getMessages($cguid, $mysqli);
}

//Get conversations
$conversations = Conversation::getConversations($mysqli);
?>
					...
					<div class="e" id="h">
						...
						<a href="">
							<div class="f k">
								...
								<div class="j fr"><?php echo convert_time(date('Y-m-d H:i:s'), true); ?></div>
								<div class="z"></div>
							</div>
						</a>
						<?php } ?>
						...
						<a href="conversations/<?php echo $conversation['contact_guid'] . "/" . $conversation['conversation_guid']; ?>#l">
							<div class="f <?php if(isset($cguid) && $conversation['conversation_guid'] == $cguid) echo "k"; ?>">
								...
								<div class="cc j fr" data-md="<?php echo convert_time($conversation['made_date']); ?>">
									<?php echo convert_time($conversation['made_date'], true); ?>
									&nbsp;
				...
				<div class="l">
					...
					<div class="n">
						...
						<div class="o <?php echo $sent ? "fr" : "fl"; ?>">
							...
							<div class="z j aa <?php echo $sent ? "fr" : "fl"; ?>" data-md="<?php echo convert_time($message['made_date']); ?>">
