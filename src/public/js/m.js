...
function a() {
    ...
    r(false, "c.php?g=" + (g.innerHTML != null ? g.innerHTML : '') + "&c=" + (c.innerHTML != null ? c.innerHTML : ''), null, null, b);
}
...
function b(v) {
    if(v != null && v != "0") {
        ...
        for(i = 0; i < v.c.length; i++) {
            x = v.c[i];
            y = x.a != '' ? (x.a + ' - ') : '';
            document.getElementById('h').insertAdjacentHTML('afterbegin', '<a href="conversations/' + x.g + '/' + x.c + '#l"><div class="f"><div class="i">' + y + x.u + '</div><div class="cc j fr" data-md="">' + h(x.d) + ' &nbsp;<a href="delete-conversation/' + x.c + '"><div class="g id" alt="Delete Conversation" title="Delete Conversation" style="width: 10px; height: 10px;"></div></a></div><div class="z"></div></a>');
        }
...
function c() {
    ...
    if(a.value.length > 0 && !/^\s+$/.test(a.value)) {
        r(true, "s.php", "g=" + document.getElementById('g').innerHTML + "&c=" + document.getElementById('c').innerHTML + "&m=" + a.value, a.value, d);
        a.value = '';
        a.select();
    }
    return false;
